...
class LibPhys_GRU:
    ...
    def train(self, X, Y, save_directory, batch_size=32, number_of_batches=None, window_size=256,
        ...
        for i in range(0, n_windows, batch_size):
            ...
            # try:
            t1 = time.time()
            self.train_with_sgd(x_train, y_train, nepoch_val, decay, track_loss, save_directory, i, save_distance=save_distance)
            print("Dataset trained in: ~%d seconds" % int(time.time() - t1))
    ...
    def train_signal(self, X, Y, signal, decay=0.95, track_loss=False, save_distance=100):
        self.train(X, Y, signal.signal_directory, signal.batch_size, signal.number_of_batches, signal.window_size,
                                    signal.learning_rate_val, signal.number_of_epochs, decay=decay, track_loss=track_loss, save_distance=save_distance)
    ...
    def train_signals(self, X, Y, signal2model, decay=0.9, track_loss=False, save_distance=100):
        ...
        # try:
        t1 = time.time()
        self.train_with_sgd(x_train, y_train, signal2model.number_of_epochs, decay, track_loss, signal2model.signal_directory, 0, save_distance=save_distance)
        print("Dataset trained in: ~%d seconds" % int(time.time() - t1))
