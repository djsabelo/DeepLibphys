...
class LibPhys_GRU:
    def __init__(self, signal_dim, hidden_dim=128, bptt_truncate=-1, signal_name=None):
        ...
        self.predicted_signal = theano.shared(name='predicted_signal', value=predicted_signal.astype(theano.config.floatX))
    ...
    def save(self, dir_name, filetag):
    ...
    def train_for_crossvalidation(self, X, Y, save_directory, filename, batch_size=32, number_of_batches=None,
        ...
        self.save(filetag=self.get_file_tag(i, -10), dir_name=save_directory)

    def train(self, X, Y, save_directory, batch_size=32, number_of_batches=None, window_size=256,
              learning_rate_val=0.05, nepoch_val=5000, decay=0.9, track_loss=False, first_index=0):
        hidden_dim = self.E.shape[0]
        signal_dim = self.E.shape[1]
    ...
    def train_with_sgd(self, x_train, y_train, nepoch_val, decay, track_loss, save_directory, dataset=None):
        num_examples_seen = 0
        ...
        for epoch in range(nepoch_val):
            ...
            if epoch == 1 and loss[0] > 1:
                ...
                if relative_loss_gradient < 0:
                    ...
                    if count_up_slope >= 5:
                        ...
                        if np.size(loss) > 3:
                            print("Mean Loss in the last {0} epochs: {1:.3f} < {2:.3f} ?" .format
                                  (10, np.mean(loss[-3:]), np.mean(loss[-6:-3])))
                            if np.mean(loss[-3:]) > np.mean(loss[-6:-3]):
                    ...
                    test_gradient = True
                    count_to_break += 1
                    print("APreparing to break, too low loss: " + str(count_to_break))
    ...
    def generate_predicted_signal(self, N, starting_signal, window_seen_by_GRU_size):
        ...
        return new_signal[1:], probabilities, state_grus, update_gate, reset_gate
