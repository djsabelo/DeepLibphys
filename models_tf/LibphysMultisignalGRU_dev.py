...
class LibphysMultisignalGRU(LibphysDNN):
    ...
    def get_common_variables(self):
        ...
        return E, U, W, V, b, c, [Ns, None, None]
    ...
    def generate_online_predicted_signal(self, X0, X1):
        x0 = tf.Variable(X0, dtype=tf.int32)
        x1 = tf.Variable(X1, dtype=tf.int32)
        self.session.run(tf.global_variables_initializer())
        return self.session.run(self.feed_forward_predict(x0, x1))
