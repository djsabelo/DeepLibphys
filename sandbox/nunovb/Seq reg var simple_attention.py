...
y_train = y[800:]
y_test = np.repeat(y[-200:], len(train))
...
x = tf.placeholder(tf.float32, [None, 2]) # [timesteps, n_input]
...
#a = tf.reshape(a, [-1, timesteps, n_features])
# Feature Network
l1f = tf.layers.dense(X, n_units, tf.nn.relu)
z = tf.layers.dense(l1f, n_features)
#z = tf.reshape(z, [-1, timesteps, n_features])
...
for epoch in range(100):
    ...
    for i in range(total_batch):
        ...
        batch_y = np.repeat(y_train[i * batch_size: i * batch_size + batch_size], len(train))
