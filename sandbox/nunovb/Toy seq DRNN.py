...
X = tf.reshape(x, [-1, 1, rnn_timesteps, n_features])


n_grus = total_time // rnn_timesteps

#gru_res = []

# tf.while_loop?
# cell + str(i) = GRUCell(gru_units, reuse=True)
with tf.variable_scope("GRU",reuse=True):
    outputs = tf.map_fn(lambda ts: tf.nn.dynamic_rnn(GRUCell(gru_units), ts, dtype=tf.float32)[0],\
                        X) # cell = GRUCell(gru_units, name='GRU1')

#outputs = tf.reshape(gru_res)
#outputs, state = tf.nn.dynamic_rnn(cell, X, dtype=tf.float32)

print("GRU:", outputs)
#print(total_time / rnn_timesteps)
outputs = tf.reshape(outputs, [-1, n_grus, gru_units]) # total_time // rnn_timesteps

# Residual blocks
with tf.variable_scope('Res_conv1d'):
    l1 = tf.layers.conv1d(outputs, n_units, 4, dilation_rate=1)
...
outputs = tf.layers.dense(l3, n_units, tf.nn.relu)
