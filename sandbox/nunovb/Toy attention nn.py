...
z = tf.reshape(z, [-1, total_time, n_units])

# Mean
#glimpse = tf.reduce_mean(z,1)#tf.multiply(a, z)#tf.reduce_sum(tf.multiply(a, z),1)
...
l2 = tf.layers.flatten(tf.layers.dense(glimpse, n_units, tf.nn.relu))
