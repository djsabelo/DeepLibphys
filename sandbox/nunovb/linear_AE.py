...
class Autoencoder:
    ...
    def fit(self, x, n_epochs=15, learning_rate=0.04, batch_size=16, display_step=1, decay=0.95, save=True, name='1', load=False):
        # tf Graph input
        ...
        self.layer_3 = tf.add(tf.matmul(self.layer_2, tf.transpose(self.weights['h2'])), self.biases['b3'])
    ...
    def get_adapt_lr(self):
        return self.sess.run(tf.cast(self.lr_vector, dtype=tf.float32))
