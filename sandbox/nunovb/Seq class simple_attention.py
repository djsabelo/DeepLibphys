...
sequences = np.array([np.array([np.float32(np.random.randint(0, 5)) for i in range(timesteps)]) for i in range(samples)])#.reshape(samples, timesteps, 1)
#print(sequences[:5])
#exit()
...
#print(outputs) # outputs should be [batch_size,num_units]

# Attention Vector
#a = tf.nn.relu(tf.matmul(x, w1) + b1)
#a = tf.nn.softmax(tf.matmul(a, out_weights) + out_bias) # softmax?

# Feature vector
#z = tf.nn.relu(tf.matmul(x, w2) + b2)
#outputs = tf.matmul(z, out_weights2) + out_bias2 # activation?

# Glimpse
#outputs = tf.multiply(a, z)
...
preds = np.array(sess.run([prediction], feed_dict={x: test})).reshape(20,9)
#print(preds)
#print(y_test)
