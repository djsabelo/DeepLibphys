...
if __name__ == "__main__":
    ...
    # filenames = [filename]
    SNRs = ["RAW"] + [str(i) for i in range(12, 8, -1)]
    filenames = [SNR_DIRECTORY + "/LOSS_TENSOR_[256]_SNR_{0}.npz".format(SNR) for SNR in SNRs]
    ...
    for filename, model, signal_batch in zip(filenames, models, signals):
        try:
            ...
            loss_tensor = RLTC.get_or_save_loss_tensor(full_path=filename, force_new=True, N_Windows=N_Windows, W=W,
                                                       models=model, test_signals=signalx, mean_tol=0.8,
                                                       overlap=0.33, mini_batch=256, std_tol=1)
    ...
    batch_size_array = np.arange(1, bs)
