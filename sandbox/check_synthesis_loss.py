...
def load_loss(models, i, j, loss_filename):
    return np.load(SYNTH_DIRECTORY + loss_filename + \
...
def get_mean_and_std_matrices(all_models, source_ind, other_ind, loss_filename):
    loss_tensor = load_loss(all_models, source_ind, source_ind, loss_filename)
    ...
    loss = [load_loss(all_models, source_ind, other_ind[0], loss_filename),
            load_loss(all_models, source_ind, other_ind[1], loss_filename)]
...
def process_graphs(all_models, source_index, others_indexes, loss_file, name):
    labels_pred = [mod.name for mod in all_models[source_index]]
    labels_true = ["Source "+name, "Other "+name, "Other Signals"]

    confusion_mean, confusion_std = get_mean_and_std_matrices(all_models, source_index, others_indexes, loss_file)
...
if __name__ == "__main__":
    ...
    all_models = [db.resp_64_models, db.emg_64_32_models, db.ecg_64_models]
    all_signals = [np.load("../data/processed/Fantasia_RESP_[64].npz")['x_train'].tolist(),
                   np.load("../data/processed/FMH_[64].npz")['x_train'].tolist(),
                   np.load("../data/processed/FANTASIA_ECG[64].npz")['x_train'].tolist()]

    i = 1
    loss_filename = "/LOSS_FOR_SYNTH_ALL"
