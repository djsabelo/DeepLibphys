...
batch_size = 128
window_size = 256
signal_directory = 'NEW_CLEANER_ECG[{0}.{1}]'.format(batch_size, window_size)
...
for file_path in full_paths:
    try:
        ...
        # plt.plot(signal)
        processed, signalx = process_dnn_signal(signal, signal_dim)
...
signals_without_noise = np.load("signals_without_noise.npz")['signals_without_noise']

# plt.show()

for i, signal in zip(range(5, len(signals_without_noise)),signals_without_noise[5:]):
