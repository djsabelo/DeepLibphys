...
signals_models = db.ecg_biometry_models[:3]

# for noisy_index in range(1,5):
#     print("Processing Biometric ECG: " + str(noisy_index))
#     # filename = '../data/validation/NOISY_INDEX_WITH_CLEAN_MODEL[{0}.{1}]'.format(noisy_index, 0)
#     filename = '../data/validation/NOISY_INDEX_WITH_NOISY_MODEL[{0}.{1}]'.format(noisy_index, 1)
#     loss_tensor = calculate_loss_tensor(filename, N_Windows, W, signals_models, noisy_index=noisy_index)


filename = '../data/validation/NOISY_INDEX_WITH_CLEAN_MODEL[{0}.{1}]'.format(1, 0)
npzfile = np.load(filename + ".npz")
