...
def calculate_roc(loss_tensor, step = 0.001, last_index=1, first_index=0):
    ...
    return roc1, roc2, scores[candidate_index], eer, scores
...
def calculate_eers(batch_size, loss_tensor, iteration):
    ...
    roc1, roc2, scores, eer_min, all_scores = calculate_roc(temp_loss_tensor, step=0.001)
    # plot_roc(roc1, roc2, eer_min)
    print("EER MIN of batch_size = {0}, iteration = {1}: {2}".format(batch_size, iteration, np.min(eer_min[0])))

    return [eer_min[0, :], iteration, batch_size, scores, roc1, roc2, all_scores]

def descompress_data(data):
    EERs, iteration, batch_size, scores, roc1, roc2, all_scores = [], [], [], [], [], [], []
    for line in data:
        EERs.append(line[0])
        iteration = line[1]
    ...
    return EERs, iteration, batch_size, scores, roc1, roc2, new_data, all_scores
...
def process_alternate_eer(loss_tensor, iterations=10, savePdf=True, SNR=1, name="", batch_size=120, fs = 250):
    ...
    for iteration in range(iterations):
        ...
        pool.close()
        EER, epoch, batch_size, scores, roc1, roc2, save_data, all_scores = descompress_data(data)
    ...
    return np.mean(EERs, axis=0).T, all_scores
...
if __name__ == "__main__":
    ...
    all_models_info = [db.mit_256]#, db.ecg_SNR_9, db.ecg_SNR_8]
    SNRs = ["RAW"]#, "9", "8"]
