...
def create_spectrograms(n_samples, window_size=2048, train_ratio=0.67, nperseg=512, noverlap=480, sxx_samples=30, im_size=60, log_spec=True):
    #plt.ion()
    pool = mp.Pool(mp.cpu_count()) # -2 if working
    return pool.starmap(get_spec, zip(sorted(glob.iglob(os.path.join(DATASET_DIRECTORY+'/*', '*_[1-2]m.mat'), recursive=True)),
                                    ...
                                    repeat(noverlap), repeat(sxx_samples), repeat(im_size)))
...
def get_spec(filename, n_samples, window_size, train_ratio, nperseg, noverlap, sxx_samples, im_size, log_spec):
    ...
    for k in range(train_windows):
        ...
        if sxx_samples == 'all':
            if log_spec:
                Sxx = resize(np.log(Sxx), (im_size, im_size))
            else:
                Sxx = resize(Sxx, (im_size, im_size))
        else:
            if log_spec:
                Sxx = resize(np.log(Sxx[:sxx_samples, :]), (im_size, im_size))
            else:
...
n_samples = 3000 # number of samples from each subject/person
...
sxx_samples_list = ['all', 150, 30]

best = 0
for i in range(6):
    returned = create_spectrograms(n_samples, sxx_samples=sxx_samples_list[i % 3], log_spec=bool(i % 2))
