...
def calculate_roc(loss_tensor, step = 0.001, last_index=1, first_index=0):
    ...
    roc1 = np.vstack((np.zeros((1, n_thresholds, np.shape(loss_tensor)[0])),
                      np.ones((1, n_thresholds, np.shape(loss_tensor)[0]))))
    roc2 = np.ones((2, n_thresholds, np.shape(loss_tensor)[0]))
    scores = []
    acc = np.zeros((n_thresholds, np.shape(loss_tensor)[0]+1))
    acc[:, -1] = thresholds
    for i in range(n_thresholds):
        if i % 100 == 0:
            print(".", end="")
        score = calculate_variables(loss_tensor, thresholds[i])
        ...
        if np.alltrue(roc1[:, i, :] == np.vstack((np.zeros((1, np.shape(loss_tensor)[0])),
                                                  np.ones((1, np.shape(loss_tensor)[0]))))):
            if np.alltrue(roc2[:, i, :] == np.ones((2, np.shape(loss_tensor)[0]))):
                break
...
def process_EER(loss_tensor, iterations=10, savePdf=True, SNR=1, name=""):
    ...
    for iteration in range(iterations):
        for batch_size in batch_size_array:
            ...
            roc1, roc2, scores, eer_min = calculate_roc(temp_loss_tensor, step=0.0001)
...
def calculate_eers(batch_size, loss_tensor, iteration):
    ...
    roc1, roc2, scores, eer_min, acc, acc_min = calculate_roc(temp_loss_tensor, step=0.0001)
    # plot_roc(roc1, roc2, eer_min)
    print("EER MIN of batch_size = {0}, iteration = {1}: {2}".format(batch_size, iteration, np.min(eer_min[0])))

    return [eer_min[0, :], iteration, batch_size, scores, roc1, roc2, acc, acc_min]

def descompress_data(data):
    EERs, iteration, batch_size, scores, roc1, roc2,  acc, acc_min = [], [], [], [], [], [], [], []
    for line in data:
        EERs.append(line[0])
        iteration = line[1]
    ...
    return EERs, iteration, batch_size, scores, roc1, roc2, new_data, acc, acc_min
...
def process_alternate_eer(loss_tensor, iterations=10, savePdf=True, SNR=1, name="", batch_size=120, fs = 250):
    ...
    all_data = []
    accs = np.zeros((N_Signals+1, len(batch_size_array), iterations))
    for iteration in range(iterations):
        pool = Pool()
        ...
        pool.close()
        EER, iteration, batch_size, scores, roc1, roc2, save_data, acc, acc_min = descompress_data(data)
        EERs[:, :, iteration] = np.array(EER)
        accs[:, :, iteration] = np.array(acc_min).T
    ...
    return np.mean(EERs, axis=0).T, accs
...
def process_all_eers(loss_quaternion, SNRs, iterations=1, loss_iteration=0, batch_size=120):
    ...
    for SNR, loss_tensor in zip(SNRs, loss_quaternion):
        EERs, accs = process_alternate_eer(loss_tensor, iterations=iterations,
                               savePdf=False, SNR=SNR, name=str(loss_iteration), batch_size=batch_size)
        mean_EERs.append(EERs)

    np.savez(SNR_DIRECTORY + "/eers.npz", EERs=mean_EERs, ACCs=accs)

    return mean_EERs, accs
...
if __name__ == "__main__":
    ...
    # noise_filename = dir_name + "/signals_without_noise.npz".format(signal_dim)
    ...
    for iteration in range(iterations):
        ...
        EERs, accurancies = process_all_eers(loss_quaternion, SNRs, iterations=100, loss_iteration=iteration, batch_size=bs)
        EERs, ACCs = np.load(SNR_DIRECTORY +"/eers.npz")['EERs'], np.load("eers.npz")['ACCs']
    ...
    for snr, SNR in zip(range(len(SNRs)), SNRs):
        ...
        for iteration in range(iterations):
            ...
            accurancies.append(new_data["acc_min"])

        all_ROCs.append(ROCs)


    np.savez(SNR_DIRECTORY + "all_EERs.npz", EERs=EERs, accurancies=accurancies)
