...
def calculate_loss_tensor(filename, Total_Windows, W, signals_models, signals=None, first_index_ratio = 0.33):
    ...
    i = 0
    first_test_index = int(len(signals[0]) * first_index_ratio)
...
def process_alternate_eer(loss_tensor, iterations=10, savePdf=True, SNR=None, name="", batch_size=120, fs = 250):
    ...
    for iteration in range(iterations):
        ...
        labels = ["ECG {0}".format(i) for i in range(1, N_Signals+1)]
        if SNR is None:
            plot_errs(EERs[:, :, iteration], seconds, labels, "Mean EER",
                      "_{0}".format(name), savePdf=savePdf)
        else:
            plot_errs(EERs[:, :, iteration], seconds, labels, "Mean EER for SNR of {0}".format(SNR),
                      "_SNR_{0}_{1}".format(SNR, name, iteration), savePdf=savePdf)
    ...
    if SNR is None:
        np.savez(SNR_DIRECTORY + "/ALL_DATA_{0}.npz".format(name), all_data=all_data)
    else:
...
if __name__ == "__main__":
    ...
    for iteration in range(iterations):
        ...
        for test_signal in test_signals:
            _, _, N_Windows, _ = segment_signal(test_signal, W, 0.33, None, 0)
            n_windows = N_Windows if N_Windows < n_windows else n_windows

        loss_tensor = calculate_loss_tensor(filename, n_windows, W, all_models_info[0], test_signals,
                                            first_index_ratio=0)
    ...
    all_data = []
    EERs = np.zeros((iterations, len(seconds)))
    all_ROCs = []

    for iteration in range(iterations):
        filename = SNR_DIRECTORY + "/ALL_DATA_{1}".format(iteration)
        all_data = np.load(filename + ".npz")["all_data"][0]
        roc1, roc2, scores, eer, thresholds, candidate_index, new_data = descompress_data(all_data)
        EERs[iteration, :] = np.mean(new_data["EERs"], axis=0)

        all_ROCs.append(roc1)


    np.savez(SNR_DIRECTORY + "all_EERs.npz", EERs=EERs)
    # EERs = np.load(SNR_DIRECTORY + "all_EERs.npz")["EERs"]
    plot_errs(np.mean(EERs, axis=1), seconds, None, "Mean EER for ALL SNR", "all_SNR", savePdf=True, plot_mean=False)