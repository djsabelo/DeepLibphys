...
ecgs = np.load("../data/signals_without_noise.npz")['signals_without_noise']

# clean_loss_tensor = calculate_loss_tensor("../data/try_CLEAN_1024", N_Windows, W, db.ecg_1024_clean_models, ecgs)
# clean_loss_tensor = np.load("../data/CLEAN_1024" + ".npz")["loss_tensor"]

# description = "NOISY_INDEX_WITHOUT_NOISE_"
description = "NOISY_INDEX_WITH_NOISE_"
ecgs = processed_noise_array[[2, 3, 4]]
SNRs = SNRs[[2, 3, 4]]


signals_models = [db.ecg_SNR_10, db.ecg_SNR_9, db.ecg_SNR_8]
loss_quartenion = []
for snr, signal_models, processed_array in zip(SNRs, signals_models, ecgs):
    "NOISY_INDEX_WITH_NOISE_"
    filename = '../data/validation/{0}{1}'.format(description, snr)
    # filename = '../data/validation/NOISY_INDEX_WITHOUT_NOISE_{0}'.format(snr)

    print("Processing Biometric ECG - with #windows of " + str(N_Windows) + " filename: " + filename)
    loss_tensor = calculate_loss_tensor(filename, N_Windows, W, signal_models, processed_array)
    loss_tensor = np.load(filename + ".npz")["loss_tensor"]
    m_labels = [model_info.name for model_info in signal_models]
    s_labels = ["ECG {0}".format(i) for i in range(1, np.shape(loss_tensor)[1]+1)]
    loss_quartenion.append(loss_tensor)
i = 0

#
# EER = process_EER(clean_loss_tensor, N_Windows, W, iterations=1, savePdf=False, SNR="NA", name="CLEAN")
#
# labels = ["NO SNR"]
# # SNRs = [snr]
# for SNR, loss_tensor in zip(SNRs, loss_quartenion):
#     print("SNR of "+str(SNR))
#     # classified_matrix = calculate_classification_matrix(calculate_min_windows_loss(loss_tensor, 6))
#     EERs = np.vstack((EERs, process_EER(loss_tensor, N_Windows, W, iterations=1, savePdf=False, SNR=SNR, name=description)))
